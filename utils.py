...
def get_window_with_active(windows):
    ws = [w for w in windows if w.is_active()]
    if ws:
        return ws[0]
    return None
...
async def await_buster(
    processes, process, timeout=120, startup_timeout=20, keepwindowon=False
):
    ...
    if not league_window:
        yield "LEAGUE WINDOW COULD NOT BE LOCATED", True
        return

    yield "FOUND WINDOW, STARTING TRACKING", False

    while 1:
        if not processes[process]:
            yield "Stopped Process", False
            return
        all_windows = get_all_windows()
        ...
        # print(cnt)
        if cnt > 10:
            yield f"POSSIBLE AFK MESSAGE DETECTED WITH {cnt} PIXELS OF COLOUR {RGB_BUSTER}", True

        if last != league_window and last:
            if not keepwindowon:
